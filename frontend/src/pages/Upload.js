...
function Upload() {
  ...
  const handleDeleteAll = async () => {
    if (window.confirm("Are you sure you want to delete all requirements?")) {
      try {
        const response = await axios.delete(`${API_URL}/api/requirements/delete-all`);
  ...
  const handleStatusUpdate = async (id, status) => {
    try {
      await axios.patch(`${API_URL}/api/requirements/${id}/status`, { status });
  ...
  const handleUpdateRequirement = async () => {
    try {
      await axios.put(`${API_URL}/api/requirements/${editingReq.id}`, {
        ...editingReq,
